...
DOCKER_NETWORK_NAME_LEAF_1_EXT="leaf1ext"
...
DOCKER_NETWORK_SUBNET_LEAF_1_EXT=10.0.5.0/24
DOCKER_NETWORK_SUBNET_LEAF_1_EXT_DEFAULT_GW=10.0.5.1
...
echo "Creating 3-Layer FRR + BGP + ECMP Labo"
...
sudo docker network create ${DOCKER_NETWORK_NAME_LEAF_1_EXT} --subnet=${DOCKER_NETWORK_SUBNET_LEAF_1_EXT}
...
sudo docker run -dit \
    -v=`pwd`/config/daemons:/etc/frr/daemons \
    -v=`pwd`/config/leaf1-ext/frr.conf:/etc/frr/frr.conf \
    -v=`pwd`/config/leaf1-ext/vtysh.conf:/etc/frr/vtysh.conf \
    --name leaf1-ext --hostname leaf1-ext --privileged --net ${DOCKER_NETWORK_NAME_LEAF_1_EXT} frrouting/frr:v7.5.0
sudo docker network connect ${DOCKER_NETWORK_NAME_SPINE} leaf1-ext
...
sudo docker network connect ${DOCKER_NETWORK_NAME_LEAF_1_EXT} server1
...
leaf1_ext_address=$(sudo docker inspect leaf1-ext | jq .[0].NetworkSettings.Networks.${DOCKER_NETWORK_NAME_LEAF_1_EXT}.IPAddress | tr -d '"')
...
echo "        +-----------+-------------+                                 |"
echo "        |                         |                                 |"
echo "  [Leaf1(${leaf1_address})]     [Leaf1-Ext(${leaf1_ext_address})]                 [Leaf2(${leaf2_address})]"
echo "        |                         |                                 |"
echo "        +-----------+-------------+                                 |"
echo "                    |                                               |"
